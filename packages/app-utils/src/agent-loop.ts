 ...
 * re-render from its own reducer. Everything app-specific — the
 * tool definitions advertised to the agent, the context payload,
 * and the tool executors — is injected via RunLoopOptions.
 */
import {
  streamAgent,
  logAgentEvent,
...
import type { ToolCallInvocation, ToolExecutionResult } from './agent-tools.js';
...
export interface RunLoopOptions {
   ...
   *  message whose content is built via the app's buildSeedPrompt(). */
  initialMessages: AgentMessage[];
  /** Tool definitions advertised to the agent in every POST.
   *  Without these the agent falls back to text-only mode. */
  toolDefinitions: AgentToolDefinition[];
  /** Build the request context (available datasets, files) sent
   *  with every POST. Called once at loop start. */
  buildContext: () => AgentContext | Promise<AgentContext>;
  /** Execute one client-side tool call and return its result.
   *  Thrown errors are caught and fed back to the agent as a
   *  failure message so it can self-correct. */
  executeToolCall: (
