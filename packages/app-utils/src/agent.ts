 ...
 *
 * The agent loop (see agent-loop.ts) consumes this stream, executes any
...
/** App-tunable agent client settings. */
export interface AgentConfig {
  /** The `surface` tag stamped on every analytics event sent via
   *  logAgentEvent(). Apps should set a distinctive value (e.g.
   *  'criblApmInvestigation') so events are attributable. */
  surface?: string;
}

let agentSurface = 'criblSearchApp';

/**
 * Configure app-level agent client settings. Call once at app
 * startup (module scope of the app's agent wrapper is a natural
 * spot) before any investigation runs.
 */
export function configureAgent(config: AgentConfig): void {
  if (config.surface) agentSurface = config.surface;
}
 ...
 * behavior to avoid divergence. The `surface` tag comes from
 * configureAgent().
 */
